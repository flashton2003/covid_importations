...
beta_fnl <- get_first_and_last_week(per_sample_intros, "20H (Beta,V2)")
beta_nc <- filter_and_plot_national_cases(national_cases, beta_fnl, "MWI")
#beta_fi <- first_identification(genomes, "20H (Beta,V2)", beta_fnl)
beta_restrictions <- filter_and_plot_restrictions(restrictions, beta_fnl, "MWI")
#beta_restrictions

beta_samples <- filter_and_plot_samples(per_sample_intros, "20H (Beta,V2)")
#beta_samples

beta_nc / beta_restrictions  / beta_samples  + plot_annotation(title = 'Beta', theme = theme(plot.title = element_text(size = 24, face = 'bold'))) + plot_layout(heights = c(1, 1, 2))
