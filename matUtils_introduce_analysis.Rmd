...
```{r main_function, fig.width=14, fig.height=10, eval=FALSE}
...
```{r troublshooting, fig.width=14, fig.height=10, eval=TRUE}
national_cases <- read_in_national_case_numbers(national_cases_handle, countries_to_include)
per_sample_output <- read_in_per_sample_intros("/Users/flashton/Dropbox/COVID/AAP_introductions/results/2022.12.02/kenyan_introductions.txt", threshold_for_plotting)
...
print(kruskal.test(cluster_size ~ voc_for_plots, data = distinct_intros))
vocs_to_plot[[1]]
make_overall_plot(per_sample_intros, vocs_to_plot[[1]], national_cases, restrictions, "KEN")
```
...
2. update the VOCs of interest to be relevant to each country
3. Plot the overall data for each country - monthly genomes (lineage), monthly national cases, daily restriction level.
4. do a cumulative frequency plot for the introductions
5. if kw test p-val < 0.05, do pairwise wilcoxon rank
6. use the plotting theshold in the other places as well
